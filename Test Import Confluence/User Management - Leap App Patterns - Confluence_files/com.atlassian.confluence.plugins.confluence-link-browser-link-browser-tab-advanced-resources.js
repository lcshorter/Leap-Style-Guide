...
define("confluence-link-browser/link-browser-tab-advanced",["jquery","ajs","confluence/api/constants","confluence-link-browser/link-object"],function(g,d,k,l){return{dialogCreatedLinkBrowserAdvanced:function(o,e){var b,f,h,j,m=function(a){a=g(a);if(a.length){var c=a.find("a:first");c.length?c.hasClass("unresolved")?(a=c.attr("shortcut-key"),c=c.attr("data-space-key"),a?f.text(d.format("{0} is not a recognised shortcut",a)):c&&f.text(d.format("{0} is not a recognised space",
c))):(a=l.fromNode(c[0],b.val()),e.setLink(a)):f.text("The markup provided is not valid link markup")}},n=function(a,c,b){d.debug("Error during conversion: textStatus = "+c+", errorThrown = "+b);f.text("An internal server error occurred")},i=e.tabs.advanced={createPanel:function(a){h=a.baseElement;b=h.find("input[name='advanced-link']");f=h.find("div[name='advanced-error']");h.find("form").keydown(function(a){13===a.keyCode&&!e.isSubmitButtonEnabled()&&a.preventDefault()});
b.keyup(function(){clearTimeout(j);f.text("");b.val()&&(j=setTimeout(function(){var a={wiki:"["+b.val()+"]",entityId:d.Meta.get("content-id"),spaceKey:d.Meta.get("space-key")};g.ajax({type:"POST",contentType:"application/json; charset=utf-8",url:k.CONTEXT_PATH+"/rest/tinymce/1/wikixhtmlconverter",data:g.toJSON(a),dataType:"text",success:m,error:n,timeout:1E4})},200))})},setLink:function(a){b.val(a);b.keyup();b.change()},getLink:function(){return b.val()},onSelect:function(){e.moveLocationPanel(h.find("form"));
